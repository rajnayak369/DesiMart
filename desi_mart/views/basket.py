...
from desi_mart.models import Product
        ...
        if product_id not in self.basket:
            self.basket[product_id] = {'quantity': 0,
                                      'price': str(product.discount_price),
                                       'title': product.title,
                                       'id': product.id,'image':product.imageLink}
...
def basket_add(request, product_id):
    ...
    return redirect('desi_mart:basket_detail')
...
def basket_detail(request):
    ...
    for item in basket:
        item['update_quantity_form'] = BasketAddProductForm(initial={'quantity': item['quantity'],
                                                                   'override': True})
    return render(request, 'desi_mart/basket.html', {'basket': basket})
